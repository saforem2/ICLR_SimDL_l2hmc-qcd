...
\begin{figure}[htpb]
   ...
   % \includegraphics[width=\textwidth]{figures/splitx8.pdf}
   % \includegraphics[width=\textwidth]{figures/splitx9.pdf}
...
boundary conditions.
%
Our target distribution \(p(\varphi)\) is defined in terms of the Wilson action as
%
\begin{equation}
   p(\varphi)\propto e^{\beta_{t} S(\varphi)}, \quad\text{where}\quad S(\varphi) = \sum_{P}1 - \cos(\varphi_{P}(x)).
   \label{eq:wilson_action}
\end{equation}
%
Here, \(\beta = 2/g_{0}^{2}\) is the inverse coupling constant of the theory, and \(\beta\rightarrow\infty\) gives the
continuum limit.
%
Here, \(\varphi_{P}\) denotes the sum of the link variables around the elementary plaquette,
%
\begin{equation}
   \varphi_{P} = \varphi_{\mu}(x) + \varphi_{\nu}(x+\hat{\mu}) - \varphi_{\mu}(x+\hat{\nu}) - \varphi_{\nu}(x) \in[0,4\pi]
\end{equation}
%
as shown in \Figref{fig:plaquette}.
%
Note that we have written the Wilson action (\Eqref{eq:wilson_action}) in terms of the link variables \(\varphi_{\mu}(x)
\in \left[-\pi, \pi\right]\)
%
%
\begin{figure}[htpb]
...
\begin{algorithm}[htpb]%
   ...
   \SetKwProg{Fn}{def}{\string:}{}%
   \SetKwFunction{Range}{range}%
   \SetKwFor{For}{\color{blue}\texttt{for}}{\string:}{}\color{black}%
   \SetKwIF{If}{ElseIf}{Else}{if}{:}{elif}{else:}{}%
   \SetKwFor{While}{while}{:}{fintq}%
   \SetKwInOut{Input}{\color{red}{\texttt{input}}\color{black}}%
   \SetKwInOut{Output}{\color{red}{\texttt{output}}\color{black}}%
   \AlgoDontDisplayBlockMarkers\SetAlgoNoEnd%
   \DontPrintSemicolon%
   ...
   \For{\(0 \leq t < N_{\mathrm{train}}\)}{%
      {}\;
      \(p_{t}(x)\propto e^{-\beta_{t}U(x)}\)\;%
      \texttt{resample:} \(v\sim\mathcal{N}(0, \mathbbm{1})\)\;%
      \texttt{resample:} \(d\sim\mathcal{U}(+,-)\)\;%
      \texttt{construct:} \(\xi \equiv (x, v, d)\)\;%
      {}\;%
      \For{\(0 \leq \ell < N_{\mathrm{LF}}\)}{%
         \texttt{propose:} \(\xi^{\prime}\leftarrow\mathbf{FL}_{\ell}^{\pm}\xi\)
      }\;
      \texttt{compute:} \(A(\xi^{\prime}|\xi) =%
      \min\left\{1,\frac{p(\xi^{\prime})}{p(\xi)}\left|\frac{\partial%
      \xi^{\prime}}{\partial \xi^{T}}\right|\right\}\)\;
   ...
   \caption{\label{algorithm:training}Training algorithm}
\end{algorithm}
\end{document}
