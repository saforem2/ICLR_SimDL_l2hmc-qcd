...
\begin{enumerate}
   \item We propose a generalized version of the L2HMC algorithm that uses different networks (with different step
      ...
      (\Eqref{eq:new_momentum_update}, \Eqref{eq:new_position_update}) indicating that these functions are allowed to
      vary.
      %
   \item We introduce a topological charge metric \(\delta_{\mathcal{Q}_{\mathbb{R}}}(\xi^{\prime}, \xi)\) defined as
      ...
      updates (\Secref{sec:l2hmc})
      %
   \item We introduce an \emph{annealing schedule} (\Secref{sec:annealing_schedule}) \({\{\beta\}}_{t=0}^{N} =
...
\begin{enumerate}
   ...
   \item We define two physical quantities, namely the  \emph{real-valued} topological charge,
      ...
      \end{align}
      % where we approximated the projection in \Eqref{eq:qz} as
      % \(\left\lfloor\varphi_{P}\right\rfloor\approx\sin\varphi_{P}\).
\end{enumerate}
