...
\begin{enumerate}
   \item Instead of using a time step to vary the network inputs, we use completely different networks for each leapfrog
      step in our trajectory \textcolor{blue}{\(\star\)}. \marginpar{\textcolor{blue}{\(\star\) WIP}}
   ...
   \item Remove \Eqref{eq:wilsonaction}, combine with \(p_{t}(x)\) at the end of \Secref{sec:annealing}, and move
      inline. \textcolor{blue}{\(\star\)}\marginpar{\textcolor{blue}{\(\star\) WIP}}
   ...
   \item Change notation for lattice gauge theory, \(\varphi_{\mu}(x)\rightarrow x_{\mu}(n)\) to be consistent with
      \Secref{sec:l2hmc}.\textcolor{blue}{\(\star\)}\marginpar{\textcolor{blue}{\(\star\) WIP}}
...
theory. \textcolor{blue}{\(\star\)}%
\marginpar{\textcolor{blue}{\(\star\) combine w/ prev. sentence}}
%
...
\end{align}
%
where (1.) \(\zeta_{v_{k}} = (x_{k}, \partial_{x}S(x_{k}))\), \(\zeta_{x_{k}} = (x_{k}, v_{k})\) are a shorthand
notation for the input passed to the respective \(s, t, q\) network functions\footnote{Note that \(\zeta_{x},
\zeta_{v}\) are subsets of the augmented state space, independent of the variable being updated.},
and (2.) we indicate the direction of the update by the superscript \(\pm\) on \(\Gamma^{\pm}_{k}, \Lambda^{\pm}_{k}\).
%
% (2.) \(\tau(k) = {\left[\cos\tfrac{2\pi k}{N_{\mathrm{LF}}}, \sin\tfrac{2\pi k}{N_{\mathrm{LF}}}\right]}\), \(k = 0, 1,
% \ldots, N_{\mathrm{LF}}\), is a discrete time variable parameterizing our trajectory,
%
...
shown (with the general transformation \(\Lambda^{\pm}\)) in \Figref{fig:splitx}.
%
\begin{figure}[htpb]
   \centering
...
%
These outputs are then summed together before being passed through a subsequent stack of \(d\) hidden layers, i.e. 
%
\begin{equation}
...
our progress through the training phase, which can be written explicitly as  \(p_{t}(x)\propto e^{-\gamma_{t} S(x)},
\quad\text{for}\quad t = 0, 1, \ldots, N\).
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
variables}, where \(x_{\mu}(n)\) denotes the link oriented in the \(\hat{\mu}\)-direction located at the site
\(n\).
%
...
\end{equation}
%
and \(x_{P} \equiv x_{\mu}(n) + x_{\nu}(n+\hat{\mu}) - x_{\mu}(n+\hat{\nu})
-x_{\nu}(n)\) is the sum of the link variables around the \(1\times1\) elementary plaquette, as shown in
\Figref{fig:plaquette}.
%
