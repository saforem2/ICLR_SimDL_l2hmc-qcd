...
\definecolor{pinkred}{HTML}{FA5477}
...
We propose the DLHMC algorithm as a generalized HMC algorithm,
      ...
      augmented leapfrog equations (\Eqref{eq:newmomentumupdate} and \Eqref{eq:newpositionupdate}), % indicating that
...
Subsequently the $k+1$-th layer uses $\xi_{k+1}=(x''_k,v''_k,d_k)$ a its input.
...
used for the updates in \Eqref{eq:newmomentumupdate} and \Eqref{eq:newpositionupdate}.
%
\begin{figure}[htpb]
   \centering
   \includegraphics[width=\textwidth]{figures/lflayer.pdf}
   % \includegraphics[width=\textwidth]{figures/lflayer3.pdf}
   \caption{\label{fig:network}Illustration of the network architecture used in \Eqref{eq:newmomentumupdate} and \Eqref{eq:newpositionupdate}.}
\end{figure}
...
We can see in \Figref{fig:autocorrvsbeta}, that the trained model consistently outperforms generic HMC across \(\beta = 2, 3, \ldots, 7\).
%
...
%
We see in \Figref{fig:topfreeze} that HMC remains stuck at a particular value of \(\mathcal{Q}_{\mathbb{Z}}\) for large sections of the simulation, whereas the trained sampler rapidly jumps between values.
%
...
the $N_{\mathrm{LF}}$ leapfrog layers.% \(k=0, 1, \ldots, N_{\mathrm{LF}}\) during the trajectory.
%
\Figref{fig:plaqsf} and \Figref{fig:hwf} show the distribution over batches of the average plaquette value and the modified energy of the
...
As can be seen in \Figref{fig:sinQf}, the real-valued topological charge exhibits mixing behavior at the middle of the deep neural network.
%
...
\begin{figure}[hbpt]
   ...
   \begin{subfigure}[b]{0.33\textwidth}
      ...
      \caption{\label{fig:plaqsf}Difference in the average plaquette, \(\langle x_{P}-x_{P}^{*}\rangle\) at each leapfrog layer.}
   \end{subfigure}
   \hfill
