...
the \(s, t, q\) functions in the momentum update equation, \Eqref{eq:new_momentum_update}, the procedure is identical},
we temporarily ignore the discrete leapfrog index \(k\), and restrict our attention to the
\(s_{x}, t_{x}, q_{x}\) functions used in the \(x\) update equation, \Eqref{eq:new_position_update}.
...
being the \emph{position} variable \(x\) in \Secref{sec:l2hmc}.
%
...
\end{equation}
%
where \(\varphi_{P}\) denotes the sum of the link variables around the elementary plaquette,
%
\begin{equation}
