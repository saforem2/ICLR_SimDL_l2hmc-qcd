...
\begin{enumerate}
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   % \item We introduce an \emph{annealing schedule} (\Secref{sec:annealing})%
   %    \({\{\gamma\}}_{t=0}^{N} = \{\gamma_{0}, \gamma_{1}, \ldots \gamma_{N}\}\), with
   %    \(\gamma_{0} < \gamma_{1} < \cdots < \gamma_{N}\), with \(|\gamma_{t}|<1\) and \(\gamma_{t+1} - \gamma_{t} \ll 1\)
   %    that is slowly varied during training (and removed entirely during inference) that scales the potential energy
   %    function in our target distribution, \(p_{t}(x)\propto e^{\gamma_{t}S(x)}\), that allows our sampler to explore
   %    previously inaccessible regions of the phase space by shrinking the height of the various energy barriers
   %    separating isolated modes.
   %    %
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   \item We apply the proposed method to a \(1+1\)-dimensional \(U(1)\) lattice gauge theory defined on a
      ...
      %
      % required to generate independent \emph{gauge configurations}.
      %
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Recently, there has been a growing interest in applying generative machine learning techniques to help improve the efficiency and applicability of different scientific simulations.
%
...
theories.\textcolor{blue}{\(\ast\)}\marginpar{\textcolor{blue}{\(\ast\) rewrite}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
shown (with the general transformation \(\Lambda^{\pm}\)) in \Figref{fig:splitx}. \textcolor{blue}{\(\ast\)}%
\marginpar{\textcolor{blue}{\(\ast\) move splitx fig to appendix?}}
%
\begin{figure}[htpb]
   \centering
   % \includegraphics[width=\textwidth]{figures/splitx10.pdf}
   \includegraphics[width=\textwidth]{figures/updated_network2.pdf}
   \caption{\label{fig:network}Illustration of a single leapfrog layer used in the \(x\) update from the \Eqref{eq:new_position_update}.}
...
Without loss of generality\footnote{%
   ...
   functions in \Eqref{eq:new_momentum_update}, the procedure is identical.
},
%
we temporarily ignore the discrete leapfrog index \(k\), and restrict our attention to the \(s_{x}, t_{x}, q_{x}\)
functions used in the \(x\) update, \Eqref{eq:new_position_update}.
%
Each of the \(s_{x}, t_{x}, q_{x}\) functions takes as input \(\zeta_{x} = (x, v)\), with \(x\in\mathbb{R}^{n}\),
\(v\in\mathbb{R}^{n}\).
%
The network first passes each of the inputs \(x\), \(v\) through a dense layer.
%
These outputs are then summed together before being passed through an additional stack of \(d\) hidden layers, i.e. \marginpar{a single ``leapfrog layer''}
%
\begin{equation}
   ...
   \,\ldots,\,%
   % \text{until: }
   % z_{n}=\sigma(w^{T}_{n}z_{n-1} + b_{n}),%
   % z_{n-1}=\sigma(w^{T}_{n-1}z_{n-2} + b_{n-1}),%
   % \ldots,%
   % z_{2} = \sigma(w^{T}_{2}z_{1} + b_{2}),
...
\marginpar{Clean up; maybe move annealing schedule section into sentence here}
%
% \begin{equation}
%    \ell_{\theta}{\left[\xi, \xi^{\prime}, A(\xi^{\prime}|\xi)\right]} =
%    -\frac{1}{a^{2}}\cdot{\delta(\xi^{\prime},\xi)\cdot A(\xi^{\prime}|\xi)},
% \end{equation}
% %
% \begin{equation}
%    \ell_{\theta}{\left[\xi, \xi^{\prime}, A(\xi^{\prime}|\xi)\right]} =
%    -\frac{\delta(\xi^{\prime},\xi)\cdot A(\xi^{\prime}|\xi)}{a^2},
% \end{equation}
% %
% \(\delta(\xi, \xi^{\prime})\) is a suitably chosen \emph{metric function}, and \(a\) is a scaling factor.
% %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\(n\).
%
% The behavior of our model is dictated by the Wilson action, \(S_{\beta}(x) = \beta\cdot\sum_{P}1-\cos(x_{P})\)
%
% We denote by \(P\) the \(1\times1\) elementary plaquette with counter-clockwise orientation as shown in
% \Figref{fig:plaquette}.
%
% Our theory is defined in terms of the Wilson action, \(S_{\beta}(x) = \beta\cdot\sum_{P}1-\cos(x_{P})\),
%
We can write our target distribution \(p_{t}(x)\) \Eqref{eq:targetannealing} in terms of the Wilson action, \(S_{\beta}(x) = \beta\cdot\sum_{P}1-\cos(x_{P})\).\marginpar{awful sentence}
%
%\begin{equation}
...
%     \centering
%     \begin{tikzpicture}
%         \draw[->, black, very thick] (0, 0) -- (1, 0) node[anchor=north] {\(x_{\mu}(n)\)};
%         \draw[black, very thick] (1, 0) -- (2, 0) {};
%         \draw[->, black, very thick] (2, 0) -- (2, 1) node[anchor=west]
%             {\(x_{\nu}(n+\hat{\mu})\)};
%         \draw[black, very thick] (2, 1) -- (2, 2) {};
%         \draw[->, black, very thick] (2, 2) -- (1, 2) node[anchor=south] {\(-x_{\mu}(n+\hat{\nu})\)};
%         \draw[black, very thick] (1, 2) -- (0, 2) {};
%         \draw[->, black, very thick] (0, 2) -- (0, 1) node[anchor=east] {\(-x_{\nu}(n)\)};
%         \draw[black, very thick] (0, 1) -- (0, 0) {};
%         \filldraw [black] (0,0) circle (2pt) node[anchor=north east] {\(n\)};
...
% \end{figure}
%
% \includegraphics[width=\textwidth]{figures/topological_freezing_blue_wide-1.png}
% \includegraphics[width=\textwidth]{figures/topological_freezing_33.pdf}
% \includegraphics[width=\textwidth]{figures/autocorr_vs_beta_blue_1504.pdf}
\begin{figure}[htpb]
   \centering
   ...
   % \begin{figure}[htpb]
   % \centering
   %(energy density for the physics we are interested in) 
   \begin{subfigure}{0.31\textwidth}
      \includegraphics[width=\textwidth]{figures/plaqsf_1758.pdf}
      \caption{\label{fig:plaqsf}Change in the average plaquette, \(\langle x_{P}-x_{P}^{*}\rangle\) vs
      leapfrog step.}
   \end{subfigure}
   \hfill
   \begin{subfigure}{0.31\textwidth}
   ...
   \begin{subfigure}{0.31\textwidth}
      ...
      \caption{Log probability of the model (includes both \(x\) and \(v\)}%
   \end{subfigure}
...
%    \centering
%    \begin{subfigure}{0.4\textwidth}
%       \includegraphics[width=\textwidth]{figures/autocorr_vs_traj_len_2152_b4.pdf}
%    \end{subfigure}
%    % \hfill
%    \begin{subfigure}{0.4\textwidth}
%       \includegraphics[width=\textwidth]{figures/autocorr_vs_traj_len_2152_b5.pdf}
%    \end{subfigure}
...
%    \centering
%    \begin{subfigure}{0.4\textwidth}
%       \includegraphics[width=\textwidth]{figures/autocorr_vs_traj_len_2152_b6.pdf}
%    \end{subfigure}
%    % \hfill
%    \begin{subfigure}{0.4\textwidth}
%       \includegraphics[width=\textwidth]{figures/autocorr_vs_traj_len_2152_b7.pdf}
%    \end{subfigure}
