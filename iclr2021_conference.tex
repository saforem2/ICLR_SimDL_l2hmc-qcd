...
%improve the efficiency applicability of different scientific simulations.
%
...
\Lambda^{\pm}_{k}\).
%
We include in \Figref{fig:network} an illustration of the network architecture used for the \(x\) update \Eqref{eq:new_position_update}.
%
\begin{figure}[htpb]
   \centering
   % \includegraphics[width=\textwidth]{figures/splitx10.pdf}
   \includegraphics[width=\textwidth]{figures/n}
...
%as shown (with the general transformation \(\Lambda^{\pm}\)) in \Figref{fig:splitx}. \textcolor{blue}{\(\ast\)}%
...
\(v\in\mathbb{R}^{n}\).
%
An illustration of the network architecture used can be seen in \Figref{fig:network}.
%
%These inputs are then passed through a \emph{leapfrog layer}, as shown in \Figref{fig:network}.
%%
...
\end{equation}
%
where (\(1\).) \(\gamma_{t}\) is a scaling factor (\(\|\gamma_{t}\|<1\)) from our annealing schedule (\Secref{sec:annealing}), and (\(2\).) \(x_{P} \equiv x_{\mu}(n) + x_{\nu}(n+\hat{\mu}) - x_{\mu}(n+\hat{\nu}) -x_{\nu}(n)\) is the sum of the link
...
%In order to calculate physical quantities, we perform analyses at some finite lattice spacing \(a\), and draw conclusions by extrapolating our results to the continuum limit (\(a\rightarrow 0\)).
%
%Unfortunately, as we decrease the lattice spacing, HMC is known to perform poorly at sampling from different values of \(\mathcal{Q}_{\mathbb{Z}}\)
%When attempting to generate configurations on inc
%This quantity is known to 
%
% \begin{figure}[htpb]
%     \centering
%     \begin{tikzpicture}
