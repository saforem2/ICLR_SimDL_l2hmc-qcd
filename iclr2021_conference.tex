...
\(v\) respectively) networks' inputs,\footnote{%
   ...
   % Note that \(\zeta_{x}, \zeta_{v}\) are subsets of the augmented state space, independent of the variable being
   % updated.
...
\begin{enumerate}
   ...
   % updates half of the components of \(x\) determined by the mask \(\mt\) followed by an analogous update of the
   % complementary components (using \(\mbart\).} %its complement \(\mbart\).}:
      \hspace{14pt} \(%
