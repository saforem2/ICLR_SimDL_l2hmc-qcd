...
which is made exact with the help of Metropolis-Hastings \citep{...} algorithm.
...
yet suffers exponential slow downs when approaching continuum physics \citep{kschaefer2009investigating,cossu2018testing}.
...
\begin{enumerate}
   \item We propose the DLHMC algorithm as a generalized L2HMC algorithm,
      using a stack of distinct neural network layers as leapfrog steps that uses different networks (with different step
...
\(v\) respectively) networks' inputs,\footnote{%
 ...
 the $x$ network will be made independent with the mask $m$ below.},
...
\begin{figure}[htpb]
   \centering
   \includegraphics[width=\textwidth]{figures/network9.pdf}
...
%
\begin{equation}
   p_{t}(x) = e^{-\gamma_{t} S_{\beta}(x)},\quad\text{with}\quad S_{\beta}(x) = \beta \sum_{P}1 - \cos(x_{P})
